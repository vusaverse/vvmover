...
############################################### #############################
## get_recent_file_date_filename_ymd() gets the most recent version of a
## file, based on the date in the file name.
...
get_recent_file_date_filename_ymd <- function(path, match){
  ...
  if (grepl("^20[0-9]{2}", basename(thisfile))) {
    ...
    print(paste("Most recent file is", basename(thisfile)))
    return(thisfile)
  }
  else
